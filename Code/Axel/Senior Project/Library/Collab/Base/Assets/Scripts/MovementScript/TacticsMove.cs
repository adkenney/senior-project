...
public class TacticsMove : MonoBehaviour
{
    ...
    protected Tiles FindEndTile(Tiles t)
    {
        ...
        {
            Debug.Log("This tile is being returned");
            return t;
    ...
    public void NextSpot(Tiles tiles)
    {
        LayerMask mask = LayerMask.GetMask("character");
        RaycastHit2D hit = Physics2D.Raycast(tiles.transform.position, Vector2.zero, .2f, mask);
        
        if (hit && tiles.target)
        {
            MoveToTile(tiles.parent);
            return;
        }
        else
        {
